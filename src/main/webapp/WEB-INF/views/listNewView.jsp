							...
							<td class="align-middle text-center" colspan="5">
								...
								<c:if test="${bookNewList.currentPage > 1}">
									<button class="button button1" type="submit"
										title="첫 페이지로 이동합니다." onclick="location.href='list?list=New&currentPage=1'">처음</button>
								</c:if> 
								<c:if test="${bookNewList.currentPage <= 1}">
								...
								<c:if test="${bookNewList.startPage > 1}">
									<button class="button button1" type="submit" title="이전 10페이지로 이동합니다."
										onclick="location.href='list?list=New&currentPage=${bookNewList.startPage - 1}'">이전</button>
								</c:if> 
								<c:if test="${bookNewList.startPage <= 1}">
								...
								<c:forEach var="i" begin="${bookNewList.startPage}" end="${bookNewList.endPage}" step="1">
									...
									<c:if test="${bookNewList.currentPage != i}">
										<button class="button button1" type="button"
											onclick="location.href='list?list=New&currentPage=${i}'">${i}</button>
									</c:if>
								</c:forEach> 
								<!-- 10페이지 뒤로 --> 
								<c:if test="${bookNewList.endPage < bookNewList.totalPage}">
									<button class="button button1" type="submit"
										...
										onclick="location.href='list?list=New&currentPage=${bookNewList.endPage + 1}'">다음</button>
								</c:if> 
								<c:if test="${bookNewList.endPage >= bookNewList.totalPage}">
									<button class="button button2" type="button" disabled="disabled"
								...
								<c:if test="${bookNewList.currentPage < bookNewList.totalPage}">
									<button class="button button1" type="submit" title="마지막 페이지로 이동합니다."
										onclick="location.href='list?list=New&currentPage=${bookNewList.totalPage}'">마지막</button>
								</c:if>
							</td>
						</tr>
					</c:if>
