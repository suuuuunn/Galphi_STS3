							...
							<td class="align-middle text-center" colspan="5">
								...
								<c:if test="${bookCategoryList.currentPage > 1}">
									<button 
										...
										type="button" 
										title="첫 페이지로 이동합니다."
										onclick="location.href='list?list=Develop&currentPage=1'"
									>처음</button>
								</c:if>
								<c:if test="${bookCategoryList.currentPage <= 1}">
									<button 
								...
								<!-- 10페이지 앞으로 -->
								<c:if test="${bookDevelopList.startPage > 1}">
									<button
										class="button button1"
										type="button"
										title="이전 10페이지로 이동합니다."
										onclick="location.href='list?list=Develop&currentPage=${bookCategoryList.startPage - 1}'"
									>이전</button>
								</c:if>
								...
								<c:if test="${bookCategoryList.currentPage != i}">
									<button
										class="button button1"
										type="button"
										onclick="location.href='list?list=Develop&currentPage=${i}'"
									>${i}</button>
								</c:if>
								...
								<c:if test="${bookCategoryList.endPage < bookCategoryList.totalPage}">
									<button
										...
										onclick="location.href='list?list=Develop&currentPage=${bookCategoryList.endPage + 1}'"
									>다음</button>
								</c:if>
								...
								<c:if test="${bookCategoryList.currentPage < bookCategoryList.totalPage}">
									<button
										...
										onclick="location.href='list?list=Develop&currentPage=${bookCategoryList.totalPage}'"
									>마지막</button>
								</c:if>
							</td>
					 	 </tr>	
